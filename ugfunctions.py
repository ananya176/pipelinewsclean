...
def mywsclean(myfile, myniter, thresh, srno, cell, imsize, int_out):
        import subprocess
        nameprefix = getfields(myfile)[0]
        print("The image files have the following prefix =", nameprefix)
        if int_out==1:
                myoutimg = nameprefix+'-combined-img'
        elif myniter==0:
                myoutimg = nameprefix+'-dirty-img'
        else:
                myoutimg = nameprefix+'-selfcal'+'img'+str(srno)
        input_ms_file=str(myfile)
        output_image_name=myoutimg
        output_image_width=imsize
        output_image_height=imsize
        niter=str(myniter)
        mythresh=thresh
        cell_size=cell
        intout=int_out
        try:
                subprocess.run(["./clean.sh", input_ms_file, output_image_name, str(output_image_width), str(output_image_height), str(niter), str(mythresh), str(cell_size), str(intout)], check=True)
                print("Bash script executed successfully.")
        except subprocess.CalledProcessError as e:
                print(f"Error running Bash script: {e}")
        return myoutimg

def calself(myfile,myref,nloops,nploops,myvalinit,mycellsize,myimagesize,mynterms2,mywproj1,mysolint1,myclipresid,myflagspw,mygainspw2,mymakedirty,niterstart,clean_robust):
        myref = myref
        nscal = nloops # number of selfcal loops
        npal = nploops # number of phasecal loops
        myimages=[]
        ...
        if nscal == 0:
                ...
                if usetclean == False:
                        ...
                        myimg = mywsclean(myfile[i],myniter,mythresh,i,mycellsize,myimagesize,5)
        else:
                for i in range(0,nscal+1): # plan 4 P and 4AP iterations
                        if mymakedirty == True:
                                if i == 0:
...
#                                        print("Using "+ myfile[i]+" for imaging.")
                                        ...
                                        if i!= nscal:
                                                ...
                                                if i < nscal+1:
                                                        ...
                                                        myfile.append(myoutfile)
                                if i == nscal:
                                        mywsclean(myfile[i],myniter,mythresh,i,mycellsize,myimagesize,1)
#                                print("Visibilities from the previous selfcal will be deleted.")
                                logging.info("Visibilities from the previous selfcal will be deleted.")
                                if i < nscal:
                                        fprefix = getfields(myfile[i])[0]
